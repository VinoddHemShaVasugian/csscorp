...
import resources.BLFunctions.BLVerification.TireQuoteByTyreSizeHelper;
 ...
 * @author css89714
 */
public class TireQuoteByTyreSize extends TireQuoteByTyreSizeHelper {
	/**
	 * Script Name : <b>TireQuoteByTyreSize</b> Generated : <b>Jul 24, 2013
	 * 3:18:15 AM</b> Description : Functional Test Script Original Host : WinNT
		...
		try {
			...
			for (int outerLoopIndex = 0; outerLoopIndex < eachTireSearchResultContainer
				...
				specificTireResultContainer.findElement(
						By.className("tire-selector-quote"))
						.click();
				...
				String tireQuote_Valve_Stem_Text = KeyWordDrivenConstants.webDriver
						.findElement(
								By.xpath("//div[@id='content']/div[2]/div[2]/table/tbody/tr[8]/td"))
						.getText().replaceAll(",", "");

				logInfo("valve_Stem_Charges = " + tireQuote_Valve_Stem_Text);

				if (false == tireQuote_Valve_Stem_Text.contains("Valve Stem")) {

					throw new BSROException(
							"Valve Stem text verification failed.");

				}

				String tireQuote_Valve_Stem_Charges = KeyWordDrivenConstants.webDriver
						.findElement(
				...
				float bill_valve_stem_charge = 0;
				...
				if (!tireQuote_Valve_Stem_Charges.contains("FREE")
						&& !tireQuote_Valve_Stem_Charges.contains("$0")) {

					tireQuote_Valve_Stem_Charges = tireQuote_Valve_Stem_Charges
							.replaceAll("\\$", "").trim();

					bill_valve_stem_charge = Float
							.parseFloat(tireQuote_Valve_Stem_Charges);

				} else {

					bill_valve_stem_charge = 0;
				...
				logInfo("bill_valve_stem_charge = " + bill_valve_stem_charge);

				float bill_total_expected = (bill_env_fee + bill_installation
						+ bill_recycle_charge + bill_tire_rotation
						+ bill_tiresPrice + bill_wheel_alignment
						+ bill_wheel_balance + bill_valve_stem_charge);
				...
				if (bill_total_actual == bill_total_expected) {
					...
					bill_total_expected = (float) (bill_total_expected + 0.01);

					bill_total_expected_formatted = "" + bill_total_expected;
