package ProductSpecificFunctions.TuranzaTires;
...
import resources.ProductSpecificFunctions.TuranzaTires.HomePageHelper;
...
public class HomePage extends HomePageHelper {
	/**
	 * Script Name : <b>HomePage</b> Generated : <b>Jul 11, 2013 2:50:00 AM</b>
	...
	private static void verifyLeftPanelTiresListContents() throws BSROException {
		...
		try {
			...
			WebElement faltTiresListinLeftPanel = KeyWordDrivenConstants.webDriver
					.findElement(By.id("turanza-tire-list"));

			java.util.List<WebElement> faltTiresWidgets = faltTiresListinLeftPanel
			...
			while (iterator.hasNext()) {
				...
				if (tireNumberToTest == loopCount) {
					...
					String vehicleType = flatTireWidget.findElement(
							By.className("turanzahead1tag")).getText();

					String tyreModel = flatTireWidget.findElement(
							By.className("hiding")).getText();
	...
	public static void verifyLeftPanelTiresList() throws BSROException {
		...
		try {
			...
			logError("FlatTires.HomePage.TuranzaTires.verifyLeftPanelTiresList() function failed.");
