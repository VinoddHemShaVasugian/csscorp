package ProductSpecificFunctions.Tires.Search.Verify.Quote;
...
import resources.ProductSpecificFunctions.Tires.Search.Verify.Quote.VerifyGetTireQuoteHelper;
 ...
 * @author css89714
 */
public class VerifyGetTireQuote extends VerifyGetTireQuoteHelper {
	/**
	 * Script Name : <b>VerifyGetTireQuote</b> Generated : <b>Jul 23, 2013
	 * 12:33:15 AM</b> Description : Functional Test Script Original Host :
	...
	public static void verify() throws BSROException {
		...
		try {
			...
			ArrayList<String> article_List_Available_In_Store = TiresSearchDBResults
					.getStandardTiresArticleListAvailableInStore();

			logInfo("article_List_Available_In_Store = "
					+ article_List_Available_In_Store);

			logInfo("article_List_Available_In_Store size = "
					+ article_List_Available_In_Store.size());

			logInfo("Web Results size = "
					+ eachTireSearchResultContainer.size());

			// if (article_List_Available_In_Store.size() !=
			// eachTireSearchResultContainer
			// .size()) {
			//
			// throw new BSROException(
			// "Search results count mismatch from web and db");
			//
			// }
			...
			for (int outerLoopIndex = 0; outerLoopIndex < eachTireSearchResultContainer
				...
				if (outerLoopIndex >= article_List_Available_In_Store.size()) {

					logInfo("Breaking loop.");
				...
				for (int innerLoopIndex = 0; innerLoopIndex < article_List_Available_In_Store
						.size(); innerLoopIndex++) {

					String articleNumber = article_List_Available_In_Store
							.get(innerLoopIndex);

					String tireFullName_DB = TiresSearchDBResults
							.getStandardTireResultantNameByArticleNumber(articleNumber);

					if (tireFullName_DB.equals(tireFullName_Web)) {
						...
						String tires_original_price = specificTireResultContainer
								.findElement(By.className("price-large"))
								.getText().replace("$", "")
								.replaceAll("\\r\\n|\\r|\\n", "").trim();
						...
						String tireQuote_Page_Title = HtmlBrowser
								.getCurrentPageTitle();
						...
						String tireQuote_ItemHeader = KeyWordDrivenConstants.webDriver
								.findElement(
										By.cssSelector("div.details.left > h2"))
								.getText();
						...
						String tireQuote_Additional_Items_Actual = KeyWordDrivenConstants.webDriver
								.findElement(
										By.cssSelector("td.additional-items"))
								.getText().replaceAll(",", "");
						...
						tireQuote_Price_Expected = tireQuote_Price_Expected
								.substring(
										0,
										tireQuote_Price_Expected.indexOf(".") + 3);

						logInfo("tireQuote_Price_Actual = "
								+ tireQuote_Price_Actual);
						...
						if (false == tireQuote_Installtion
								.equals("Installation")) {
						...
						if (false == tireQuote_TPMS
								.contains("TPMS Valve Service Kit")) {
						...
						logInfo("tireQuote_Grand_Total text = "
								+ tireQuote_Grand_Total);

						if (false == tireQuote_Grand_Total.contains("TOTAL")) {

							throw new BSROException(
									"Total text verification failed.");
						...
						if (!tireQuote_Tire_Rotation_Charges.contains("FREE")
								&& !tireQuote_Tire_Rotation_Charges
										.contains("$0")) {
						...
						if (!tireQuote_Wheel_Balancing_Charges.contains("FREE")
								&& !tireQuote_Wheel_Balancing_Charges
										.contains("$0")) {
						...
						if (!tireQuote_TPMS_Charges.contains("FREE")
							...
							tireQuote_TPMS_Charges = tireQuote_TPMS_Charges
									.replaceAll("\\$", "").trim();
						...
						if (!tireQuote_Tyre_Recycling_Charges.contains("FREE")
								&& !tireQuote_Tyre_Recycling_Charges
										.contains("$0")) {
						...
						logInfo("bill_wheel_alignment = "
								+ bill_wheel_alignment);
						...
						float bill_total_expected = (bill_env_fee
								+ bill_installation + bill_recycle_charge
								+ bill_tire_rotation + bill_tiresPrice
								+ bill_tpms_value_kit
								+ bill_tpms_value_kit_labour
								+ bill_wheel_alignment + bill_wheel_balance);

						String bill_total_expected_formatted = ""
								+ bill_total_expected;

						bill_total_expected_formatted = bill_total_expected_formatted
								.substring(0, bill_total_expected_formatted
										.indexOf(".") + 3);
						...
						if (bill_total_actual == bill_total_expected) {
							...
							float diffAmount = (bill_total_actual - bill_total_expected);

							float diffAmountFormatted = Float.parseFloat(String
									.format("%.2f", diffAmount));

							logInfo("diffAmount = " + diffAmountFormatted);

							if (Math.rint(Double.parseDouble(""
									+ diffAmountFormatted)) == Math.rint(Double
									.parseDouble("" + 0.01))) {
								...
								throw new BSROException(
										"Tire quote bill verification failed , diff amount = "
												+ diffAmountFormatted);
			...
			logError("VerifyGetTireQuote.verify() function failed.");
