...
public class dbConnections {
	...
	public static void openDBConnection()
	{
		try
		{
			...
			String url = "jdbc:mysql://10.50.35.8:3306/automation?autoReconnect=true&useSSL=false";

			DBConstants.dBConnectionProperties=new Properties();

			DBConstants.dBConnectionProperties.setProperty("user","monty");

			DBConstants.dBConnectionProperties.setProperty("password","root");
...
//			String query=" SELECT STEPS.ID,STEPS.stepid,STEPS.obj_ref_id,obj.obj_ref_name,obj.target_property,STEPS.testcaseid, STEPS.htmlelementtype,ACTION.actiontypename,STEPS.status,STEPS.valuetext FROM test_case_design_steps STEPS inner join actiontype ACTION on ACTION.ID=STEPS.actiontype_id  inner join obj_repository obj on STEPS.obj_ref_id =  obj.id where  STEPS.testcaseid = (SELECT ID FROM testcases WHERE testcasename ='"+Constants.TC_Name+"')ORDER BY STEPS.stepid ASC";
			String query=" SELECT STEPS.ID,STEPS.stepid,STEPS.obj_ref_id,obj.obj_ref_name,obj.target_property,STEPS.testcaseid, STEPS.htmlelementtype,ACTION.actiontypename,STEPS.status,STEPS.valuetext,stepds.datasetname,stepds.datasetvalues FROM test_case_design_steps STEPS inner join actiontype ACTION on ACTION.ID=STEPS.actiontype_id  inner join obj_repository obj on STEPS.obj_ref_id =  obj.id left join steptestdataset stepds on stepds.id=steps.datasetid  where  steps.status<>0 and STEPS.testcaseid = (SELECT ID FROM testcases WHERE testcasename ='"+Constants.TC_Name+"')ORDER BY STEPS.stepid ASC";
			...
			while(queryResult.next())
			{
				...
				String ddname=queryResult.getString(11);
				String ddvalues=null;
				ddvalues=queryResult.getString(12);
			
				System.out.println("");
				if(ddvalues!=null){
				Datadriven dd=new Datadriven();
				dd.setDataDrivenDatasetName(ddname);
				dd.setValues(ddvalues.split((",")));
				Constants.tcStepDatadriven.add(dd);
				}
			}
		}
		catch(Exception ex)
