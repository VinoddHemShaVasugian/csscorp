...
public class CommonActions {
	...
	public static boolean checkDDTest(){
		
		if(Constants.tcStepDatadriven.size()!=0){
...
public static  boolean ValidateDD(){
	int l=Constants.tcStepDatadriven.get(0).getValues().length;
	for(int i=1;i<Constants.tcStepDatadriven.size();i++){
		if(l!=Constants.tcStepDatadriven.get(i).getValues().length){
		return false;
		}
...
//tcActualDataSetValue
public static void storeDD_ActualValue(){
	
	
	Constants.tcActualDataSetValue.add(Constants.sValue);
	ResultLogger.log("Stored DD succefully.", ISSTEPACTION.True, RESULT.PASS);
//	Constants.tcStepDatadriven.get(Constants.iteration).
//	int l=Constants.tcStepDatadriven.get(0).getValues().length;
//	for(int i=1;i<=Constants.tcStepDatadriven.size()-1;i++){
//		if(l!=Constants.tcStepDatadriven.get(i+1).getValues().length){
//		return false;
		}
...
public static String[] getSpecifiedDataSetValues(String dsName){
	String[] ddvalues=null;
	for(int i=0;i<=Constants.tcStepDatadriven.size()-1;i++){
		if(dsName.equals(Constants.tcStepDatadriven.get(i).getDataDrivenDatasetName())){
			ddvalues=Constants.tcStepDatadriven.get(i).getValues();
			break;
		}
	}
	return ddvalues;
	...
	public static void clickAndWait()
	{
		try
		{
			...
			if(Constants.Browser_Name.toUpperCase().equals("FIREFOX"))
				Thread.sleep(2000);
...
//			if(!Constants.OSType.toUpperCase().equals("IOS")){
			...
			tempKeySplitter = StorageArea.getHashTable(Constants.sValue);
...
//			else{
////				String str = EntryPoint.driver.executeScript("seetest:client.elementGetText(\"NATIVE\", Constants.webElement)");
//				Constants.sActualValue = (String) EntryPoint.AppDriver.executeScript("seetest:client.elementGetText(\"NATIVE\", Constants.webElement, \"0\")");
//								
//			}
			...
			{
				 ResultLogger.log("ACTUAL VALUE:"+Constants.sActualValue+"<br/>Expected Value:"+tempKeySplitter+"Actual and Expected texts are equal.",ISSTEPACTION.True,RESULT.PASS);
			}
			else
			{
				ResultLogger.log("ACTUAL VALUE:"+Constants.sActualValue+"<br/>Expected Value:"+tempKeySplitter+"Actual and Expected texts are equal not equal.",ISSTEPACTION.True,RESULT.WARNING);
			}
...
//
	...
	public static void webElementPresent()
	{
		try
		{
//			ObjectHandler.splitTarget();
	...
	public static void SelectValueInCalendar()
	{
		try
		{
			ObjectHandler.GetWebElement();
			...
			if (IsElementPresent())
			{
				((JavascriptExecutor)EntryPoint.driver).executeScript("arguments[0].value=''",Constants.webElement);
				
				((JavascriptExecutor)EntryPoint.driver).executeScript("arguments[0].value='"+Constants.sValue+"'",Constants.webElement);
//				Constants.webElement.sendKeys(StorageArea.getHashTable(Constants.sValue));
		...
		{
			ResultLogger.log("Exception occured at SelectValueInCalendar ", ISSTEPACTION.True,RESULT.EXCEPTION);
	...
	public static void verifyRequiredAttribute()
	{
		String tempKeySplitter = "";
		try
		{
...
//			if(!Constants.OSType.toUpperCase().equals("IOS")){
//			Constants.sActualValue = Constants.webElement.getAttribute("required");
			Constants.sActualValue = Constants.webElement.getAttribute("validationMessage");
...
//			}
			...
			{
				ResultLogger.log("ACTUAL VALUE:"+Constants.sActualValue+"<br/>Expected Value:"+tempKeySplitter+"Actual and Expected texts are not equal.",ISSTEPACTION.True,RESULT.WARNING);
			}
