	...
	public static void launchBrowser()
	{
		try
		{
			...
			if(Constants.OSType.toUpperCase().equals("WINDOWS") && Constants.Browser_Name.toUpperCase().equals("chrome".toUpperCase()) )
			{
				...
				System.setProperty("webdriver.chrome.driver","D:\\Automation\\WorkSpace\\EL_Project\\lib\\Drivers\\chromedriver.exe");
...
//					capabilities.setCapability("chrome.switches", Arrays.asList("--incognito"));
				EntryPoint.driver = new ChromeDriver(capabilities);
			}
			...
			if(Constants.OSType.toUpperCase().equals("ANDROID"))
			{
				...
				EntryPoint.capabilities.setCapability("deviceName", "430045aeb402c0f1"); //HTC
				...
				// Set android VERSION desired capability. Set your mobile device's OS version.
        				EntryPoint.capabilities.setCapability(CapabilityType.VERSION, "7.0");
...
//					EntryPoint.capabilities.setCapability("deviceName","MaryKayUser's iPad");
					...
					EntryPoint.capabilities.setCapability(MobileCapabilityType.UDID, "9e4cf18b059325be5602dbe73f5b7594ad77a615");
					EntryPoint.capabilities.setBrowserName(MobileBrowserType.SAFARI);
			...
			{
				System.setProperty("webdriver.gecko.driver", "D:\\Automation\\WorkSpace\\EL_Project\\lib\\Drivers\\geckodriver.exe");
...
//				System.setProperty("webdriver.ie.driver","./lib/Drivers/IEDriverServer.exe");
				System.setProperty("webdriver.ie.driver","D:\\Automation\\WorkSpace\\EL_Project\\lib\\IE_Driver_32bit\\IEDriverServer.exe");
//				System.setProperty("webdriver.ie.driver","./lib/IE_Driver_32bit/IE9-Windows7-x86-enu.exe");
...
//ieCapabilities.setCapability("acceptInsecureCerts",true);
//ieCapabilities.setAcceptInsecureCerts(true);
ieCapabilities.acceptInsecureCerts();
//ieCapabilities.setCapability(CapabilityType.ACCEPT_SSL_CERTS, true);

EntryPoint.driver  = new InternetExplorerDriver(ieCapabilities);
...
//				EntryPoint.driver = new InternetExplorerDriver(caps);
				...
				//EntryPoint.driver = new InternetExplorerDriver();

			}
			else if(Constants.Browser_Name.toUpperCase().equals("SAFARI"))
			{
				EntryPoint.driver = new SafariDriver();
			...
			{
//				if(Constants.OSType.toUpperCase().equals("Windows".toUpperCase()))
//				EntryPoint.driver.manage().window().maximize();
				EntryPoint.driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
				EntryPoint.driver.manage().timeouts().pageLoadTimeout(120, TimeUnit.SECONDS);
				EntryPoint.driver.manage().timeouts().setScriptTimeout(30, TimeUnit.SECONDS);
	...
	public static void start_driver(int time)
	{
		...
		while(System.currentTimeMillis() < end)
		{
			if(Constants.OSType.toUpperCase().equals("ANDROID"))
			{
				try
				{
					EntryPoint.AppDriver = new AndroidDriver<WebElement>(new URL("http://127.0.0.1:4723/wd/hub"),EntryPoint.capabilities);
...
//			EntryPoint.driver.navigate().to(Constants.sValue);
			EntryPoint.driver.get(Constants.sValue);
...
//			
			if(EntryPoint.driver.getTitle().contains("Certificate")){
				EntryPoint.driver.navigate ().to ("javascript:document.getElementById('overridelink').click()");
			}
		}
		catch(Exception ex)
		{
