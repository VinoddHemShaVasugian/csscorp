...
public class EntryPoint {
	...
	public static void start() throws Exception{
		...
		for(Constants.stepNumber=0;Constants.stepNumber <= Constants.Actions.size()-1;Constants.stepNumber++)
		{
			...
			Constants.Step_info = "Action: "+Constants.sAction+"|| Target: "+Constants.sTarget;
			...
			if(Constants.sValue.startsWith("@{")){
				String []ddValues=CommonActions.getSpecifiedDataSetValues(Constants.sValue.substring(2, Constants.sValue.length()-1));
				Constants.sValue=ddValues[Constants.iteration];
			...
			ResultLogger.log("Step Number:"+(Constants.stepNumber+1)+"|Action:"+Constants.sAction+"|Target:"+Constants.sTarget+"|Value:"+Constants.sValue, ISSTEPACTION.False, RESULT.PASS);
...
//		Constants.watch.stop();
	...
	public static void main(String[] args) {
		...
		try
		{
			...
			Constants.isDDTest=CommonActions.checkDDTest();

			if(Constants.isDDTest){
			//Initialize the webDriver
				if(CommonActions.ValidateDD()){
					
				
					
					for(int i=0;i<=Constants.tcStepDatadriven.get(0).getValues().length-1;i++){
//						Constants.watch.start();
						Browser.launchBrowser();
					start();
					Constants.iteration++;
					Constants.tcActualDataSetValue.clear();

						Constants.stepLog.StepAction.clear();
						Constants.stepLog.StepTarget.clear();
						Constants.stepLog.StepValue.clear();
						Constants.stepLog.StepComments.clear();
						Constants.stepLog.StepResult.clear();
						
					}

				}
				else{
					ResultLogs.ResultLogger.log("Invalid DD test", ISSTEPACTION.False, RESULT.FAIL);
				}
	...
	public static void InitializeTestExecution(String[]args){
		...
		try
		{
			...
			if(Constants.isLocalExecution)
			{
				...
				Constants.OSType = "WINDOWS".toUpperCase();  //OSType: WINDOWS ANDROID IOS
				
				Constants.Browser_Name = "Chrome".toUpperCase(); //Browser Names: Chrome / IE / Firefox / Safari

				Constants.TC_Name = "Registration"; //Provide the Test case name to run PriceFilter_VerifyProducts//SelectStoreAndPlaceOrder_Test//Android_AddProductToBag
//				POC_Prod_Desktop_Signup - Android Tab, Desktop (Chrome, Firefox and IE).
//				POC_PROD_Desktop_ContactUs - Android Tab, Desktop (Chrome, Firefox and IE).
//				POC_Prod_Mobile_Signup - Andorid phone
//				POC_Prod_Tablet_ContactUs - 
...
//				}
	...
	public static void executeAction() throws Exception
	{
		

		Browser.waitForPageLoad();

		if(Constants.sAction.toUpperCase().equals("dropdown_selectByVisibleText".toUpperCase()))
		{
			ListBoxActions.dropdownSelectedValueByVisibleText();
		}
		else if(Constants.sAction.toUpperCase().equals("OPEN"))
		{
			Browser.open();
		}
		...
		else if(Constants.sAction.toUpperCase().equals("verifyElementPresent".toUpperCase()))
		{
			CommonActions.webElementPresent();
		}
		else if(Constants.sAction.toUpperCase().equals("Select_Date_Calendar".toUpperCase()))
		{
			CommonActions.SelectValueInCalendar();
		}
		else if(Constants.sAction.toUpperCase().equals("storeDD_ActualValue".toUpperCase()))
		{
			CommonActions.storeDD_ActualValue();
		}
		else if(Constants.sAction.toUpperCase().equals("verifyRequiredAttribute".toUpperCase()))
		{
			CommonActions.verifyRequiredAttribute();
		}
		else if(Constants.sAction.toUpperCase().equals("verifyRecord".toUpperCase()))
		{
			ProductSpecificActions.verifyRecord();
		}
		}
}
